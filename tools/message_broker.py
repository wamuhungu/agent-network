...
class MessageBroker:
    ...
    def _setup_infrastructure(self):
        ...
        try:
            ...
            # Setup queues with verification
            ...
            for queue_name in queues:
                ...
                # Bind with explicit routing key
                ...
                # Verify binding
                self.logger.info(f"Queue {queue_name} bound to {self.EXCHANGE_NAME}")
    ...
    def publish_message(self, queue_name: str, message: Dict[str, Any]) -> bool:
        """
        Enhanced publish with timeout and retry.
        ...
        try:
            ...
            self.logger.error(f"Publish error for {queue_name}: {e}")
            return False
        
        return False
    
    def _publish_direct(self, queue_name: str, message: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Direct publish successful to {queue_name}")
            return True
        except Exception as e:
            self.logger.error(f"Direct publish failed to {queue_name}: {e}")
            return False

    def send_message(self, queue_name: str, message: Dict[str, Any]) -> bool:
        """
        Enhanced send with fallback to direct publishing.
        ...
        try:
            ...
            self.logger.warning(f"Queue validation failed for {queue_name}: {e}")
        
        # Try exchange-based first
        success = self.publish_message(queue_name, message)
        if not success:
            self.logger.warning(f"Exchange-based publish failed for {queue_name}, trying direct publish...")
            # Fallback to direct queue publishing
            return self._publish_direct(queue_name, message)
        return success
